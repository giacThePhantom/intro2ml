...
L'algoritmo cerca una soluzione nello spazio di ipotesi.
	...
	Guida perrtanto la selezione della migliore soluzione in $\mathcal{F}_{task}$.
	Pertanto:
	...
	Le funzioni di generalizzazione e di training error possono essere scritte in termini di una pointwise loss $\mathcal{l}(f;z)$ che misura l'errore che avviene a $f$ su un esempio di training $z$.
	$$E(f;P_{data})=\mathbb{E}_{z\sim P_{data}}[\mathcal{l}(f;z)]$$
	$$E(f;\mathcal{D}_n)=\dfrac{1}{n}\sum\limits_{i=1}^n\mathcal{l}(f;z_i)$$
	...
	L'errore di geenralizzazione pu\`o essere stimati utilizzando diversi insiemi di training, validation e test.
